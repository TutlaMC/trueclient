      ...
      <LiquidGlass className="flex-1 overflow-y-auto divide-y divide-white/10 space-y-2">
        {mods.map((mod) => (
          <LiquidGlass className="flex items-center justify-between p-3">
            <span className="text-white/90 font-medium">{mod}</span>
            <button onClick={()=>{window.electronAPI.deleteMod(mod)}} className="p-2 rounded-full bg-red-500/80 hover:bg-red-500 transition cursor-target">
